...
export default class UserCrud extends Component {
    ...
    getUpdatedList(user, add = true){
        const list = this.state.list.filter(i => i.id !== user.id)
        if(add) list.unshift(user)
        return list
    }

    updateField(event) {
        const userInside = {...this.state.user}
        userInside[event.target.name] = event.target.value
        this.setState({ user: userInside })
    }
    ...
    remove(user) {
        axios.delete(`${baseUrl}/${user.id}`).then(response => {
            const newList = this.getUpdatedList(user, false)
